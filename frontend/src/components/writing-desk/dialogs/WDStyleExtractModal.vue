...
import type { Chapter } from '@/api/types/novel'
...
function getChapterNumber(ch: Chapter): number | null {
...
const emit = defineEmits<{
  (e: 'close'): void
  (e: 'extracted', summary: StyleSummary | null): void
}>()
...
async function loadChapters() {
  try {
    ...
    availableChapters.value = (res.chapters || [])
      .filter((ch: Chapter) => ch.content && ch.content.length > 500)
      .map((ch: Chapter) => {
...
async function startExtract() {
  ...
  try {
    ...
    error.value = (e as Error).message || pick('提取失败，请重试', 'Extraction failed. Please try again.')
...
async function handleCreateSource() {
  ...
  try {
    ...
    error.value = (e as Error).message || pick('保存参考文本失败', 'Failed to save the reference text')
  } finally {
    loading.value = false
  }
}
...
async function handleDeleteSource(sourceId: string) {
  ...
  try {
    ...
    error.value = (e as Error).message || pick('删除参考文本失败', 'Failed to delete the reference text')
  } finally {
    loading.value = false
  }
}
...
async function handleCreateProfile() {
  ...
  try {
    ...
    error.value = (e as Error).message || pick('生成文风画像失败', 'Failed to generate the style profile')
  } finally {
    loading.value = false
  }
}
...
async function handleActivateProfile(profileId: string) {
  ...
  try {
    ...
    error.value = (e as Error).message || pick('激活文风画像失败', 'Failed to activate the style profile')
  } finally {
    loading.value = false
  }
}
...
async function handleClearActiveProfile() {
  ...
  try {
    ...
    error.value = (e as Error).message || pick('停用失败', 'Failed to disable the style')
  } finally {
    loading.value = false
  }
}
