...
> 面向长篇小说、系列叙事与复杂世界观创作的本地优先 AI 写作工程平台。

玄穹文枢聚焦“**灵感采集 → 蓝图建模 → 章节生成 → 多轮评审 → 知识沉淀**”的完整创作链路，不把 AI 当作一次性续写工具，而是把它纳入可管理、可验证、可复用的内容生产系统。项目当前已具备项目蓝图、章节写作台、LLM 多配置编排、写作技能调度、线索追踪、知识图谱、一致性分析与验证闭环等核心能力。

> 本 README 已合并原 `READ.md` 的导航与验证说明。原仓库宣传内容、二维码、外部截图、在线体验链接和历史仓库口径已移除。

## 产品定位

玄穹文枢是一个**面向网络小说作者、内容工作室与长篇叙事团队的 AI 协同创作基础设施**。

它解决的不是“怎么多写一点字”，而是长篇创作里更难的几个问题：

- 如何让灵感、人物、设定、章节、伏笔和版本演化保持同一套上下文。
- 如何让生成结果可审查、可比较、可回滚，而不是一次性黑盒输出。
- 如何把长文本创作从“即时交互”升级为“工程化生产流程”。

## 一句话简介

你构建的是一个**以叙事一致性控制为核心的 AI 小说生产系统**：它能把灵感收集、蓝图确认、章节生成、版本比对、线索回收、角色关系维护和质量评审串成一条完整的创作流水线，并通过知识索引与配置治理，让长篇内容生成变得更稳定、更可控、更适合持续迭代。

## 核心价值

- **工程化创作**：把小说创作从零散对话式调用，升级为具备任务、状态、资产和验证机制的长期生产流程。
- **一致性控制**：围绕人物、设定、伏笔、知识图谱与记忆层，降低长篇创作中的设定漂移、线索断裂和上下文遗忘。
- **模型治理**：支持 LLM 多配置组、健康检查、来源追踪与切换策略，避免模型接入完全失控。
- **过程可审计**：章节生成、版本差异、评审结果、运行日志与验证脚本形成可复盘证据链。
- **本地优先落地**：适合个人作者、工作室化写作和需要私有化掌控创作资产的场景。

## 主要能力

### 1. 创作主链路
- 灵感模式：概念澄清、蓝图确认、结构化收口
- 项目蓝图：世界观、角色、地点、派系与章节规划
- 写作台：章节生成、重生成、评审、确认、编辑、版本管理

### 2. 叙事资产管理
- 角色、关系、势力、地点与世界设定统一管理
- 线索追踪、伏笔管理、情绪曲线、知识图谱联动分析
- 章节记忆、上下文检索与项目知识沉淀

### 3. 模型与运行治理
- LLM 多配置组、健康检查、来源追踪、自动切换
- 写作技能目录、安装 / 卸载、执行建议
- 运行日志、验证脚本、回归检查与部署口径管理

## 适用场景

- 需要持续产出长篇连载内容的个人作者
- 多人协同、希望统一设定与节奏控制的写作工作室
- 希望把 AI 写作能力产品化、流程化、平台化的团队
- 需要本地优先、可私有部署、可审计的叙事生成系统

## 系统摘要

从当前仓库能力看，玄穹文枢已经形成了从前端工作台、后端编排服务、模型配置治理到本地验证闭环的完整系统骨架。它不仅能承担章节级内容生成，还能通过状态追踪、多层评审与知识索引，对创作过程实施过程化校准，显著提升长篇叙事系统的稳定性、可控性与交付质量。
...
## 托管式验证与审计工具（2026-04-29）

```powershell
# 端到端冒烟：SQLite 隔离库、后端健康、前端首页、蓝图状态、浏览器截图/乱码/console 检查
.\tools\dev-smoke.ps1

# 已生成小说/数据库/输出 JSON 深度审计，生成 Markdown + JSON 报告
python .\tools\audit_generated_novels.py

# 历史章节生成状态修复：默认 dry-run；确认后 --apply，脚本会先备份 SQLite 主库
python .\tools\repair_generation_state.py
python .\tools\repair_generation_state.py --apply
```

关键报告：

- `docs/reports/function-tree-managed-2026-04-29.md`
- `docs/reports/generated-novel-quality-audit-2026-04-29.md`
- `docs/reports/managed-progress-2026-04-29.md`

导出安全口径：TXT/DOCX 导出前会强制校验章节必须 `successful`、必须有选中版本且正文非空；否则返回 `409 / novel_export_not_ready`，避免导出空章或半成品。

新增验证脚本：

```powershell
# 灵感/蓝图/章节正文/写作桌面/导出 TXT 的 Playwright 全链路 E2E
.\tools\e2e-inspiration-to-export.ps1

# 剩余导出阻断章节恢复稿修复，默认 dry-run，--apply 前会备份主库
python .\tools\repair_export_blockers.py
python .\tools\repair_export_blockers.py --apply
```
...
RETRY_BASE_INTERVAL=2  # 基础重试间隔秒数（默认：2）
BACKEND_TIMEOUT=10     # 后端 API 超时时间秒数（默认：10）
FRONTEND_TIMEOUT=10    # 前端超时时间秒数（默认：10）
SLOW_THRESHOLD=8       # 慢响应阈值秒数（默认：8）
```
...
```bash
# 快速检查（适合 CI/CD）
MAX_RETRIES=2 BACKEND_TIMEOUT=5 ./healthcheck.sh

# 宽松检查（适合故障排查）
MAX_RETRIES=10 RETRY_BASE_INTERVAL=5 BACKEND_TIMEOUT=30 ./healthcheck.sh
```
...
```bash
# 在 Cron 中运行，仅在失败时告警
* * * * * /path/to/healthcheck.sh || notify-pagerduty "玄穹文枢健康检查失败"
```
