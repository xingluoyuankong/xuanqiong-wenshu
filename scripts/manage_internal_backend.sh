...
write_manifest() {
  ...
  mkdir -p "$STATE_DIR"
  XQ_PID="$pid" XQ_STATUS="$status" XQ_PORT="$PORT" XQ_ROOT="$ROOT" XQ_LOG="$LOG" XQ_ERR="$ERR" XQ_APP="$APP" XQ_PYTHON="$XQ_PYTHON" XQ_COMMIT="$(git -C "$ROOT" rev-parse HEAD 2>/dev/null || true)" XQ_STARTED="${XQ_STARTED:-}" XQ_PYTHONPATH="$PYTHONPATH" \
  "$XQ_PYTHON" - <<'PY'
...
status() {
  ...
  if [[ -n "$pid" ]]; then
    XQ_STATE_DIR="$STATE_DIR" XQ_HOST="$HOST" XQ_PID="$pid" XQ_STATUS="running" XQ_PORT="$PORT" XQ_ROOT="$ROOT" XQ_LOG="$LOG" XQ_ERR="$ERR" XQ_APP="$APP" XQ_PYTHON="$XQ_PYTHON" XQ_COMMIT="$(git -C "$ROOT" rev-parse HEAD 2>/dev/null || true)" XQ_PYTHONPATH="$PYTHONPATH" write_manifest "$pid" running >/dev/null
  else
    XQ_STATE_DIR="$STATE_DIR" XQ_HOST="$HOST" XQ_PID=0 XQ_STATUS="stopped" XQ_PORT="$PORT" XQ_ROOT="$ROOT" XQ_LOG="$LOG" XQ_ERR="$ERR" XQ_APP="$APP" XQ_PYTHON="$XQ_PYTHON" XQ_COMMIT="$(git -C "$ROOT" rev-parse HEAD 2>/dev/null || true)" XQ_PYTHONPATH="$PYTHONPATH" write_manifest 0 stopped >/dev/null
  fi
  cat "$MANIFEST"
...
start() {
  ...
  for _ in $(seq 1 60); do
    if curl -fsS --max-time 2 "http://${HOST}:${PORT}/api/health" >/dev/null 2>&1; then
      XQ_STATE_DIR="$STATE_DIR" XQ_HOST="$HOST" XQ_PID="$(listener_pid)" XQ_STATUS=running XQ_PORT="$PORT" XQ_ROOT="$ROOT" XQ_LOG="$LOG" XQ_ERR="$ERR" XQ_APP="$APP" XQ_PYTHON="$XQ_PYTHON" XQ_COMMIT="$(git -C "$ROOT" rev-parse HEAD 2>/dev/null || true)" XQ_PYTHONPATH="$PYTHONPATH" write_manifest "$(listener_pid)" running >/dev/null
