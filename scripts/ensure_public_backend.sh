...
if grep -q '"status": "running"' "$LOG_DIR/status.out" && ! grep -q '"code_drift": true' "$LOG_DIR/status.out" || grep -q '"working_tree_dirty": true' "$LOG_DIR/status.out" && ! grep -q '"working_tree_dirty": true' "$LOG_DIR/status.out"; then log "public backend healthy and commit-aligned"; exit 0; fi
