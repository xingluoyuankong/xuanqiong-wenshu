...
if grep -q '"status": "running"' "$LOG_DIR/status.out" && ! grep -q '"code_drift": true' "$LOG_DIR/status.out"; then log "public backend healthy and commit-aligned"; exit 0; fi
if grep -q '"code_drift": true' "$LOG_DIR/status.out"; then log "public backend code drift detected; restarting"; "$MANAGER" restart >>"$LOG_FILE" 2>&1; exit 0; fi
