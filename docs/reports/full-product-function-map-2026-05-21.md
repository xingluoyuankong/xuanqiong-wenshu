...
## 2026-05-21 继续收口：伏笔下章任务闭环
- `foreshadowing.py` 列表接口不再只返回内容和状态，也会返回已有模型里的回收计划、揭示方式、影响、相关角色/剧情、重要性、紧迫度和关键词。
- `ForeshadowingSection.vue` 基于这些字段新增“下章必须处理”工作区，把逾期、提醒待办和高紧迫伏笔变成明确任务，并输出局部补丁建议，避免用户只能看到“待回收数量”却不知道下一章该补哪一段。
- 架构边界：没有新增伏笔表或线索表，仍复用现有 `Foreshadowing` / `ForeshadowingReminder` 账本，只补齐 API 暴露和前端任务化视图。
