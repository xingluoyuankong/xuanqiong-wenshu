# Auto File Index

- Generated at (UTC): `2026-04-09T11:12:33.953601+00:00`
- Project root: `<repo-root>`

## Zone Summary

| Zone | Files | Description |
| --- | ---: | --- |
| `backend-api` | 19 | FastAPI 路由层，承接请求并调用服务层。 |
| `backend-core` | 11 | 配置、依赖、数据库会话和初始化。 |
| `backend-domain` | 40 | ORM 模型、Schema、Repository。 |
| `backend-service` | 61 | 写作流水线、LLM 调度、业务服务与异步任务。 |
| `backend-utils` | 9 | 工具函数、第三方适配与辅助脚本。 |
| `frontend-shell` | 6 | 前端入口、路由、状态管理。 |
| `frontend-ui` | 80 | 页面与组件层。 |
| `frontend-data` | 8 | API 客户端、组合式逻辑和前端工具。 |
| `ops-deploy` | 14 | 部署配置、容器、启动器和迁移脚本。 |
| `docs-and-guides` | 19 | 说明文档、审计报告、优化报告。 |
| `other` | 62 | 未命中分区规则的文件 |

## Zone Details

### Backend API Layer (`backend-api`)

- `backend/app/api/routers/__init__.py`
- `backend/app/api/routers/admin.py`
- `backend/app/api/routers/analytics.py`
- `backend/app/api/routers/analytics_enhanced.py`
- `backend/app/api/routers/clue_tracker.py`
- `backend/app/api/routers/foreshadowing.py`
- `backend/app/api/routers/knowledge_graph.py`
- `backend/app/api/routers/llm_config.py`
- `backend/app/api/routers/novels.py`
- `backend/app/api/routers/optimizer.py`
- `backend/app/api/routers/outline.py`
- `backend/app/api/routers/patch_diff.py`
- `backend/app/api/routers/projects.py`
- `backend/app/api/routers/review.py`
- `backend/app/api/routers/style.py`
- `backend/app/api/routers/token_budget.py`
- `backend/app/api/routers/updates.py`
- `backend/app/api/routers/writer.py`
- `backend/app/api/routers/writing_skills.py`

### Backend Core & Infra (`backend-core`)

- `backend/app/config/__init__.py`
- `backend/app/config/celery_config.py`
- `backend/app/core/__init__.py`
- `backend/app/core/config.py`
- `backend/app/core/dependencies.py`
- `backend/app/core/security.py`
- `backend/app/db/__init__.py`
- `backend/app/db/base.py`
- `backend/app/db/init_db.py`
- `backend/app/db/session.py`
- `backend/app/db/system_config_defaults.py`

### Backend Domain Models (`backend-domain`)

- `backend/app/models/__init__.py`
- `backend/app/models/admin_setting.py`
- `backend/app/models/chapter_blueprint.py`
- `backend/app/models/clue_tracker.py`
- `backend/app/models/constitution.py`
- `backend/app/models/faction.py`
- `backend/app/models/foreshadowing.py`
- `backend/app/models/knowledge_graph.py`
- `backend/app/models/llm_config.py`
- `backend/app/models/memory_layer.py`
- `backend/app/models/novel.py`
- `backend/app/models/outline_alternative.py`
- `backend/app/models/patch_diff.py`
- `backend/app/models/project_memory.py`
- `backend/app/models/prompt.py`
- `backend/app/models/system_config.py`
- `backend/app/models/token_budget.py`
- `backend/app/models/update_log.py`
- `backend/app/models/usage_metric.py`
- `backend/app/models/user.py`
- `backend/app/models/user_daily_request.py`
- `backend/app/models/writer_persona.py`
- `backend/app/models/writing_skill.py`
- `backend/app/repositories/__init__.py`
- `backend/app/repositories/admin_setting_repository.py`
- `backend/app/repositories/base.py`
- `backend/app/repositories/llm_config_repository.py`
- `backend/app/repositories/novel_repository.py`
- `backend/app/repositories/prompt_repository.py`
- `backend/app/repositories/system_config_repository.py`
- `backend/app/repositories/update_log_repository.py`
- `backend/app/repositories/usage_metric_repository.py`
- `backend/app/repositories/user_repository.py`
- `backend/app/schemas/__init__.py`
- `backend/app/schemas/admin.py`
- `backend/app/schemas/config.py`
- `backend/app/schemas/llm_config.py`
- `backend/app/schemas/novel.py`
- `backend/app/schemas/prompt.py`
- `backend/app/schemas/user.py`

### Backend Service Pipeline (`backend-service`)

- `backend/app/services/__init__.py`
- `backend/app/services/admin_setting_service.py`
- `backend/app/services/ai_review_service.py`
- `backend/app/services/blueprint_service.py`
- `backend/app/services/cache_service.py`
- `backend/app/services/chapter_context_service.py`
- `backend/app/services/chapter_guardrails.py`
- `backend/app/services/chapter_ingest_service.py`
- `backend/app/services/chapter_review_service.py`
- `backend/app/services/character_knowledge_manager.py`
- `backend/app/services/clue_tracker_service.py`
- `backend/app/services/config_service.py`
- `backend/app/services/consistency_service.py`
- `backend/app/services/constitution_service.py`
- `backend/app/services/creative_guidance_system.py`
- `backend/app/services/embedding_service.py`
- `backend/app/services/emotion_analyzer_enhanced.py`
- `backend/app/services/emotion_curve_service.py`
- `backend/app/services/emotion_service.py`
- `backend/app/services/enhanced_writing_flow.py`
- `backend/app/services/enrichment_service.py`
- `backend/app/services/export_service.py`
- `backend/app/services/faction_service.py`
- `backend/app/services/finalize_service.py`
- `backend/app/services/foreshadowing_service.py`
- `backend/app/services/foreshadowing_tracker_service.py`
- `backend/app/services/import_service.py`
- `backend/app/services/knowledge_graph_service.py`
- `backend/app/services/knowledge_retrieval_service.py`
- `backend/app/services/llm_config_service.py`
- `backend/app/services/llm_service.py`
- `backend/app/services/memory_layer_service.py`
- `backend/app/services/novel_service.py`
- `backend/app/services/outline_evolution_service.py`
- `backend/app/services/outline_rewriter.py`
- `backend/app/services/pacing_controller.py`
- `backend/app/services/patch_diff_service.py`
- `backend/app/services/pipeline_orchestrator.py`
- `backend/app/services/preview_generation_service.py`
- `backend/app/services/prompt_service.py`
- `backend/app/services/prompt_templates_optimized.py`
- `backend/app/services/reader_simulator_service.py`
- `backend/app/services/root_cause_diagnostics_service.py`
- `backend/app/services/self_critique_service.py`
- `backend/app/services/six_dimension_review_service.py`
- `backend/app/services/story_trajectory_analyzer.py`
- `backend/app/services/style_rag_service.py`
- `backend/app/services/test_phase4_integration.py`
- `backend/app/services/test_root_cause_diagnostics_service.py`
- `backend/app/services/token_budget_service.py`
- `backend/app/services/ultimate_writing_flow.py`
- `backend/app/services/update_log_service.py`
- `backend/app/services/usage_service.py`
- `backend/app/services/vector_store_service.py`
- `backend/app/services/vector_store_service_ext.py`
- `backend/app/services/writer_context_builder.py`
- `backend/app/services/writer_persona_service.py`
- `backend/app/services/writing_skills_service.py`
- `backend/app/tasks/__init__.py`
- `backend/app/tasks/emotion_tasks.py`
- `backend/app/tasks/foreshadowing_tasks.py`

### Backend Utilities (`backend-utils`)

- `backend/app/utils/__init__.py`
- `backend/app/utils/emotion_analyzer.py`
- `backend/app/utils/json_utils.py`
- `backend/app/utils/llm_tool.py`
- `backend/scripts/diagnose_llm_connectivity.py`
- `backend/scripts/e2e_foreshadowing_graph_validation.py`
- `backend/scripts/e2e_outline_validation.py`
- `backend/scripts/e2e_style_memory_validation.py`
- `backend/scripts/e2e_token_clue_validation.py`

### Frontend App Shell (`frontend-shell`)

- `frontend/src/App.vue`
- `frontend/src/main.ts`
- `frontend/src/router/index.ts`
- `frontend/src/stores/auth.ts`
- `frontend/src/stores/notification.ts`
- `frontend/src/stores/novel.ts`

### Frontend UI Layer (`frontend-ui`)

- `frontend/src/components/BlueprintCard.vue`
- `frontend/src/components/BlueprintConfirmation.vue`
- `frontend/src/components/BlueprintDisplay.vue`
- `frontend/src/components/BlueprintEditModal.vue`
- `frontend/src/components/ChapterList.vue`
- `frontend/src/components/ChapterOutlineEditor.vue`
- `frontend/src/components/CharactersEditor.vue`
- `frontend/src/components/CharactersEditorEnhanced.vue`
- `frontend/src/components/ChatBubble.vue`
- `frontend/src/components/ConversationInput.vue`
- `frontend/src/components/CustomAlert.vue`
- `frontend/src/components/EmotionBeatSelector.vue`
- `frontend/src/components/FactionsEditor.vue`
- `frontend/src/components/GlobalNavBar.vue`
- `frontend/src/components/InspirationLoading.vue`
- `frontend/src/components/KeyLocationsEditor.vue`
- `frontend/src/components/LLMSettings.vue`
- `frontend/src/components/LayeredOptimizer.vue`
- `frontend/src/components/ProjectCard.vue`
- `frontend/src/components/RelationshipsEditor.vue`
- `frontend/src/components/TheWelcome.vue`
- `frontend/src/components/Tooltip.vue`
- `frontend/src/components/TypewriterEffect.vue`
- `frontend/src/components/WelcomeItem.vue`
- `frontend/src/components/admin/NovelManagement.vue`
- `frontend/src/components/admin/PromptManagement.vue`
- `frontend/src/components/admin/RootCauseDiagnostics.vue`
- `frontend/src/components/admin/SettingsManagement.vue`
- `frontend/src/components/admin/Statistics.vue`
- `frontend/src/components/admin/UpdateLogManagement.vue`
- `frontend/src/components/clue-tracker/ClueTrackerView.vue`
- `frontend/src/components/icons/IconCommunity.vue`
- `frontend/src/components/icons/IconDocumentation.vue`
- `frontend/src/components/icons/IconEcosystem.vue`
- `frontend/src/components/icons/IconSupport.vue`
- `frontend/src/components/icons/IconTooling.vue`
- `frontend/src/components/knowledge-graph/KnowledgeGraphView.vue`
- `frontend/src/components/novel-detail/ChapterOutlineSection.vue`
- `frontend/src/components/novel-detail/ChaptersSection.vue`
- `frontend/src/components/novel-detail/CharactersSection.vue`
- `frontend/src/components/novel-detail/EmotionCurveSection.vue`
- `frontend/src/components/novel-detail/ForeshadowingSection.vue`
- `frontend/src/components/novel-detail/OverviewSection.vue`
- `frontend/src/components/novel-detail/RelationshipsSection.vue`
- `frontend/src/components/novel-detail/WorldSettingSection.vue`
- `frontend/src/components/shared/GlobalNotification.vue`
- `frontend/src/components/shared/NovelDetailShell.vue`
- `frontend/src/components/writing-desk/dialogs/WDEditChapterModal.vue`
- `frontend/src/components/writing-desk/dialogs/WDEvaluationDetailModal.vue`
- `frontend/src/components/writing-desk/dialogs/WDEvolveOutlineModal.vue`
- `frontend/src/components/writing-desk/dialogs/WDGenerateChapterModal.vue`
- `frontend/src/components/writing-desk/dialogs/WDGenerateOutlineModal.vue`
- `frontend/src/components/writing-desk/dialogs/WDMemoryManageModal.vue`
- `frontend/src/components/writing-desk/dialogs/WDPatchDiffModal.vue`
- `frontend/src/components/writing-desk/dialogs/WDSkillSelectorModal.vue`
- `frontend/src/components/writing-desk/dialogs/WDStyleExtractModal.vue`
- `frontend/src/components/writing-desk/dialogs/WDTextReaderModal.vue`
- `frontend/src/components/writing-desk/dialogs/WDTokenBudgetModal.vue`
- `frontend/src/components/writing-desk/dialogs/WDVersionDetailModal.vue`
- `frontend/src/components/writing-desk/index.ts`
- `frontend/src/components/writing-desk/layout/WDHeader.vue`
- `frontend/src/components/writing-desk/layout/WDSidebar.vue`
- `frontend/src/components/writing-desk/layout/WDWorkspace.vue`
- `frontend/src/components/writing-desk/workspace/content/ChapterContent.vue`
- `frontend/src/components/writing-desk/workspace/index.ts`
- `frontend/src/components/writing-desk/workspace/review/VersionSelector.vue`
- `frontend/src/components/writing-desk/workspace/states/ChapterEmpty.vue`
- `frontend/src/components/writing-desk/workspace/states/ChapterFailed.vue`
- `frontend/src/components/writing-desk/workspace/states/ChapterGenerating.vue`
- `frontend/src/components/writing-desk/workspace/states/WorkspaceInitial.vue`
- `frontend/src/views/AboutView.vue`
- `frontend/src/views/AdminNovelDetail.vue`
- `frontend/src/views/AdminView.vue`
- `frontend/src/views/HomeView.vue`
- `frontend/src/views/InspirationMode.vue`
- `frontend/src/views/NovelDetail.vue`
- `frontend/src/views/NovelWorkspace.vue`
- `frontend/src/views/SettingsView.vue`
- `frontend/src/views/WorkspaceEntry.vue`
- `frontend/src/views/WritingDesk.vue`

### Frontend Data & Client Logic (`frontend-data`)

- `frontend/src/api/admin.ts`
- `frontend/src/api/config.ts`
- `frontend/src/api/llm.ts`
- `frontend/src/api/novel.ts`
- `frontend/src/api/updates.ts`
- `frontend/src/composables/useAlert.ts`
- `frontend/src/utils/chapterContent.ts`
- `frontend/src/utils/date.ts`

### Deploy & Runtime (`ops-deploy`)

- `backend/db/migrations/add_chapter_outline_metadata.sql`
- `backend/db/migrations/add_deep_optimization_features.sql`
- `backend/db/migrations/add_novel_kit_features.sql`
- `backend/db/schema.sql`
- `deploy/Dockerfile`
- `deploy/docker-compose.yml`
- `deploy/docker-entrypoint.sh`
- `deploy/scripts/deploy_docker.sh`
- `deploy/scripts/quick_deploy.sh`
- `deploy/scripts/rollback.sh`
- `deploy/scripts/run_migrations.sh`
- `deploy/scripts/server_deploy.sh`
- `deploy/scripts/verify_migration.sh`
- `start_xuanqiong_wenshu.cmd`

### Documentation (`docs-and-guides`)

- `README-en.md`
- `README.md`
- `docs/RAG.md`
- `docs/architecture_fusion.md`
- `docs/code-index/README.md`
- `docs/code-index/auto-file-index.json`
- `docs/code-index/auto-file-index.md`
- `docs/code-index/functional-zones.md`
- `docs/coze_video_workflow.md`
- `docs/novel_workflow.md`
- `docs/optimization-report-2026-03-26.md`
- `docs/reports/CODE_AUDIT_REPORT.md`
- `docs/reports/CODE_REVIEW_20260329.md`
- `docs/reports/DEEP_OPTIMIZATION_REPORT.md`
- `docs/reports/INTEGRATION_REPORT.md`
- `docs/reports/NOVEL_KIT_FUSION_REPORT.md`
- `docs/reports/OPTIMIZATION_REPORT_20260329.md`
- `docs/reports/integration_design.md`
- `docs/systemic-deep-optimization-2026-03.md`

### Other (`other`)

- `.github/workflows/xuanqiong-wenshu-quick-smoke.yml`
- `DEPLOYMENT_GUIDE.md`
- `DEPLOYMENT_GUIDE_FULL.md`
- `READ.md`
- `backend/app/__init__.py`
- `backend/app/api/__init__.py`
- `backend/app/main.py`
- `backend/ark_models.json`
- `backend/bigmodel_models.json`
- `backend/bigmodel_test_results.json`
- `backend/model_test_results.json`
- `backend/modelscope_models.json`
- `backend/openrouter_models.json`
- `backend/prompts/chapter_plan.md`
- `backend/prompts/character_dna_guide.md`
- `backend/prompts/concept.md`
- `backend/prompts/constitution_check.md`
- `backend/prompts/editor_review.md`
- `backend/prompts/evaluation.md`
- `backend/prompts/extraction.md`
- `backend/prompts/faction_context.md`
- `backend/prompts/foreshadowing_reminder.md`
- `backend/prompts/import_analysis.md`
- `backend/prompts/optimize_dialogue.md`
- `backend/prompts/optimize_environment.md`
- `backend/prompts/optimize_psychology.md`
- `backend/prompts/optimize_rhythm.md`
- `backend/prompts/outline_generation.md`
- `backend/prompts/rewrite_guardrails.md`
- `backend/prompts/screenwriting.md`
- `backend/prompts/six_dimension_review.md`
- `backend/prompts/writer_persona.md`
- `backend/prompts/writing.md`
- `backend/prompts/writing_v2.md`
- `backend/pytest.ini`
- `backend/start_backend.cmd`
- `backend/test_all_models.py`
- `backend/test_bigmodel.py`
- `backend/test_free_models.py`
- `backend/test_model_iq.py`
- `backend/test_models_parallel.py`
- `backend/test_ollama_quality.py`
- `check_db.py`
- `frontend/.prettierrc.json`
- `frontend/components.d.ts`
- `frontend/env.d.ts`
- `frontend/package-lock.json`
- `frontend/package.json`
- `frontend/postcss.config.js`
- `frontend/start_frontend_dev.cmd`
- `frontend/tsconfig.app.json`
- `frontend/tsconfig.json`
- `frontend/tsconfig.node.json`
- `frontend/vite.config.ts`
- `start.ps1`
- `stop.ps1`
- `tools/generate_code_index.py`
- `tools/generate_poem_video.py`
- `tools/smoke_api_routes.py`
- `tools/smoke_llm_settings_health.py`
- `verify.ps1`
- `优化计划.md`
