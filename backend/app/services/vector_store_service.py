...
# 标志位：避免重复打印向量库未启用的警告
_vector_store_disabled_warned = False
...
class VectorStoreService:
    ...
    def __init__(self) -> None:
        global _vector_store_disabled_warned
        if not settings.vector_store_enabled:
            if not _vector_store_disabled_warned:
                logger.warning("未开启向量库配置，RAG 检索将被跳过。")
                _vector_store_disabled_warned = True
            self._client = None
    ...
    async def delete_by_project(self, project_id: str) -> None:
        """根据项目 ID 彻底删除关联的所有上下文向量数据。"""
        if not self._client or not project_id:
            return
        ...
        try:
            await self._client.execute(chunk_sql, {"project_id": project_id})  # type: ignore[union-attr]
            await self._client.execute(summary_sql, {"project_id": project_id})  # type: ignore[union-attr]
            logger.info("已彻底清空项目向量数据: project_id=%s", project_id)
        except Exception as exc:  # pragma: no cover
            logger.error("清空项目向量数据失败: project_id=%s error=%s", project_id, exc)
