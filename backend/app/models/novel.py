...
class NovelProject(Base):
    ...
    # 额外补充的业务子表映射，确保级联删除完整性
    factions: Mapped[list["Faction"]] = relationship(
        "Faction", backref="project", cascade="all, delete-orphan"
    )
    foreshadowings: Mapped[list["Foreshadowing"]] = relationship(
        "Foreshadowing", backref="project", cascade="all, delete-orphan"
    )
    foreshadowing_analysis: Mapped[Optional["ForeshadowingAnalysis"]] = relationship(
        "ForeshadowingAnalysis", backref="project", cascade="all, delete-orphan", uselist=False
    )
    causal_chains: Mapped[list["CausalChain"]] = relationship(
        "CausalChain", backref="project", cascade="all, delete-orphan"
    )
    character_states: Mapped[list["CharacterState"]] = relationship(
        "CharacterState", backref="project", cascade="all, delete-orphan"
    )
    timeline_events: Mapped[list["TimelineEvent"]] = relationship(
        "TimelineEvent", backref="project", cascade="all, delete-orphan"
    )
    story_time_tracker: Mapped[Optional["StoryTimeTracker"]] = relationship(
        "StoryTimeTracker", backref="project", cascade="all, delete-orphan", uselist=False
    )
...
class ChapterOutline(Base):
    ...
    __table_args__ = (
        UniqueConstraint("project_id", "chapter_number", name="uq_chapter_outlines_project_chapter_number"),
    )
...
class Chapter(Base):
    ...
    __table_args__ = (
        UniqueConstraint("project_id", "chapter_number", name="uq_chapters_project_chapter_number"),
    )
